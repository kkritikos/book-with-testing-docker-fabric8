...
public class FirstIT implements TestLifecycleLogger
{
    ...
    {
        given().accept("text/html").get("/rest/books").then().assertThat().contentType("text/html").and().statusCode(200);
    }
    
    @Test
    @Order(3)
    public void getBookWhileNoOneExists() throws Exception{
    	given().accept("text/html").get("/rest/books/book/xxx").then().assertThat().statusCode(404);
    }    
}
